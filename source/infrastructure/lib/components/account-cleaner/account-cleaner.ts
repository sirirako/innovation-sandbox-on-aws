...
import { ArnFormat, Aspects, Aws, CfnCondition, Duration, Fn, Stack } from "aws-cdk-lib";
import {
...
export class AccountCleaner extends Construct {
  constructor(scope: Construct, id: string, props: AccountCleanerProps) {
    ...
    // Add permission to describe Step Function executions for cleanup status checking
    initializeCleanupLambda.lambdaFunction.addToRolePolicy(
      new PolicyStatement({
        ...
        resources: [
          Stack.of(this).formatArn({
            ...
            resourceName: "AccountCleanerStepFunctionStateMachine*:*",
          }),
        ],
      }),
    );
