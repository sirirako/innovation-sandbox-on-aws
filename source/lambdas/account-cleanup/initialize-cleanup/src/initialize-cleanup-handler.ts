...
async function stepFunctionIsStillRunning(
  ...
  try {
    ...
    // If the execution doesn't exist, treat it as not running
    if (error.name === "ExecutionDoesNotExist" || error.message?.includes("Execution Does Not Exist")) {
      logger.info(`Previous cleanup execution ${executionArn} no longer exists, treating as not running`);
      return false;
    }
